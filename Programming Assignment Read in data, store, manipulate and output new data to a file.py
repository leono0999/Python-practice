...
write_first_line_to_file('hello world','sampletext2.txt')
...
def read_even_numbered_lines(file_name):
    ...
    """
    ### WRITE SOLUTION HERE
def red_even_numbered_lines():
    even_lines=[]
    with open('file.txt','r') as file:
        files=file.readlines()
        for i in range(1,len(files),2):
            even_lines.append(files[i])
    return even_lines
print(red_even_numbered_lines())
...
def read_file_in_reverse():
    reverse=[]
    with open('file.txt','r') as items:
        item = items.readlines()
